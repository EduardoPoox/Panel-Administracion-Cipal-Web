(function($) {
  "use strict";
	...
	$(document).on('click', '#bt_nv_usario', function(event) {
		...
		cargar_modal_div('usuario_nuevo.php');	
    ...
    $(document).on('submit', '#fr_usuario', function(event) {	
    	...
    	if ($(this)[0].checkValidity() === false) {
        	...
        	$.ajax({
	            url: 'controller/usuario_nuevo.php',
	            ...
	            success: function(response){
	            	switch (response) {
						  case '1':
						  		recargar_tabla('#ct_tb_usuarios','usuario_listar.php','#tb_usuarios');
	                 			$('#modal').modal('hide');
...
//CAMBIAR CONTRASEÑA USUARIO FORMULARIO BD
    $(document).on('submit', '#fr_con_usuario', function(event) {	
    	...
    	if ($(this)[0].checkValidity() === false) {
        	...
        	$.ajax({
	            url: 'controller/usuario_cambiar_contrasenia.php',
...
//EDITAR USUARIO FORMULARIO BD
    $(document).on('submit', '#fr_ed_usuario', function(event) {	
    	...
    	if ($(this)[0].checkValidity() === false) {
        	...
        	$.ajax({
	            url: 'controller/usuario_editar.php',
...
function usuario_eliminar($id,$usuario){
	...
	Swal.fire({
                ...
                if (result.value) {
                   	$.ajax({
			            url: 'controller/usuario_eliminar.php',
			            data: {id:$id},
			            dataType: "html",
			            type: 'POST',
			            success: function(response){
